...
def deep_q_learning(
    world, episodes=10000, step_limit=2000, epsilon=0.5, gamma=1, learning_rate=10**-9
):
    Q = net.BaseNet()
    episode_progress = []
    actions = [0, 1, 2, 3]
    ...
    for episode in range(episodes):
        ...
        for step in range(step_limit):
                ...
                with torch.no_grad():
                    current_action = np.argmax(
                        [
                            Q(
                                torch.from_numpy(
                                    np.append(current_state[:5], 0)
                                ).double()
                            ),
                            Q(
                                torch.from_numpy(
                                    np.append(current_state[:5], 1)
                                ).double()
                            ),
                            Q(
                                torch.from_numpy(
                                    np.append(current_state[:5], 2)
                                ).double()
                            ),
                            Q(
                                torch.from_numpy(
                                    np.append(current_state[:5], 3)
                                ).double()
                            ),
                        ]
                    )
            ...
            ep_return += reward
            with torch.no_grad():
                y = reward + gamma * np.max(
                    [
                        Q(torch.from_numpy(np.append(new_state[:5], 0)).double()),
                        Q(torch.from_numpy(np.append(new_state[:5], 1)).double()),
                        Q(torch.from_numpy(np.append(new_state[:5], 2)).double()),
                        Q(torch.from_numpy(np.append(new_state[:5], 3)).double()),
                    ]
                )
            # print(current_state[:5], current_action)
            output = Q(
                torch.from_numpy(np.append(current_state[:5], current_action)).double()
            )
        ...
        print(final_step)
        print(
            f"Episode {episode + 1} terminated after {final_step} steps. Total "
